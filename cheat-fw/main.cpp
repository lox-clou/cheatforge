// CheatForge v10 — single-thread message pump + overlay + draggable menu
#include <cstdint>
#include <cstring>
#include <cmath>
#include <vector>
...
static bool on(const char*id){return g_selected.find(id)!=g_selected.end();}
...
// ===== MENU =====
static bool g_menuOpen=true;static int g_activeTab=0;static int g_hoverIdx=-1;static HWND g_menu=0;
...
#ifdef GAME_CS2
 g_feats={{"m_aim","Silent Aim",0},{"m_head","Head Priority",0},{"m_smooth","Human Smooth",0},{"m_fov","FOV Limit",0},{"m_trig","Triggerbot",0},{"m_rcs","Recoil Ctrl",0},
  {"m_esp","Player ESP",1},{"m_hp","Health Bars",1},{"m_dist","Distance",1},{"m_radar","World Radar",1},{"d_fovc","FOV Circle",1},{"d_cross","Crosshair",1},{"d_trac","Tracers",1},{"d_hitm","Hitmarker",1},
  {"m_bhop","Bunny Hop",2},{"m_strafe","Auto Strafe",2},
  {"m_noflash","No Flash",3},{"i_rapid","Rapid Fire",3},{"d_water","Watermark",3},{"d_fps","FPS Meter",3},{"d_timer","Session Timer",3}};
#elif defined(GAME_VALORANT)
 g_feats={{"c_aim","Aimbot",0},{"c_head","Head Priority",0},{"c_smooth","Smooth",0},{"c_trig","Triggerbot",0},{"i_rec","Recoil Macro",0},
  {"c_esp","ESP Outlines",1},{"c_radar","Radar",1},{"d_fovc","FOV Circle",1},{"d_cross","Crosshair",1},{"d_hitm","Hitmarker",1},
  {"i_rapid","Rapid Fire",3},{"d_water","Watermark",3},{"d_fps","FPS Meter",3}};
#elif defined(GAME_RUST)
 g_feats={{"i_rec","Recoil AK",0},{"i_rak","AK Pattern",0},{"i_rm4","M4 Pattern",0},{"i_rapid","Rapid Fire",0},
  {"c_ore","Ore ESP",1},{"d_cross","Crosshair",1},{"d_water","Watermark",1},
  {"i_bhop","Bunny Hop",2},{"i_strafe","Air Strafe",2},{"i_tap","Tap Strafe",2},{"d_fps","FPS Meter",3}};
#elif defined(GAME_APEX)
 g_feats={{"c_aim","Aimbot",0},{"c_head","Head Priority",0},{"c_trig","Triggerbot",0},{"i_rec","Recoil Macro",0},{"i_tap","Tap Strafe",0},
  {"c_esp","ESP Glow",1},{"c_radar","Radar",1},{"d_cross","Crosshair",1},{"i_bhop","Bhop",2},{"i_strafe","Strafe",2},{"d_water","Watermark",3},{"d_fps","FPS Meter",3}};
#elif defined(GAME_PUBG)
 g_feats={{"c_aim","Aimbot",0},{"i_rec","M416 Recoil",0},{"i_rak","AKM Pattern",0},{"c_trig","Triggerbot",0},
  {"c_esp","ESP",1},{"c_radar","Radar",1},{"d_cross","Crosshair",1},{"i_rapid","Rapid Fire",3},{"d_water","Watermark",3}};
#elif defined(GAME_FORTNITE)
 g_feats={{"i_rec","AR Recoil",0},{"i_rak","AR Pattern",0},{"i_rm4","SMG Pattern",0},{"i_rapid","Rapid Fire",0},
  {"d_cross","Crosshair",1},{"d_water","Watermark",1},{"i_bhop","Bhop",2},{"i_strafe","Strafe",2},{"d_fps","FPS Meter",3}};
#elif defined(GAME_GTA5)
 g_feats={{"i_rec","Recoil",0},{"i_rapid","Rapid Fire",0},{"i_aimkey","Aim Spam",0},{"d_cross","Crosshair",1},{"d_water","Watermark",1},{"i_bhop","Jump Spam",2},{"d_fps","FPS Meter",3}};
#elif defined(GAME_MINECRAFT)
 g_feats={{"c_aim","Aimbot",0},{"c_trig","Triggerbot",0},{"i_rapid","CPS Boost",0},{"c_esp","Nametag ESP",1},{"c_ore","Ore ESP",1},{"c_radar","Radar",1},{"i_bhop","Bhop",2},{"d_water","Watermark",3},{"d_fps","FPS Meter",3}};
#elif defined(GAME_TF2)
...
static void drawMenuTo(HDC dc){
 ...
 HPEN bo=CreatePen(PS_SOLID,2,RGB(0,240,255));HGDIOBJ op=SelectObject(dc,bo);HGDIOBJ ob=SelectObject(dc,GetStockObject(NULL_BRUSH));
 ...
 SetTextColor(dc,RGB(110,116,140));TextOutA(dc,160,16,"external | INSERT hide",22);
 ...
 int tabW=MENU_W/4-4;
 for(int t=0;t<4;t++){int x=4+t*(tabW+4);RECT tr={x,44,x+tabW,70};bool act=t==g_activeTab;
  HBRUSH tb=CreateSolidBrush(act?RGB(0,240,255):RGB(26,29,40));FillRect(dc,&tr,tb);DeleteObject(tb);
  SetTextColor(dc,act?RGB(0,0,0):RGB(160,166,190));TextOutA(dc,tr.left+12,tr.top+7,TABN[t],(int)strlen(TABN[t]));}
 HPEN dv=CreatePen(PS_SOLID,1,RGB(35,38,52));HGDIOBJ dvp=SelectObject(dc,dv);MoveToEx(dc,10,82,0);LineTo(dc,MENU_W-10,82);SelectObject(dc,dvp);DeleteObject(dv);
 int y=94,idx=0;
 for(auto&f:g_feats){if(f.tab!=g_activeTab){idx++;continue;}
  ...
  HBRUSH sbg=CreateSolidBrush(isOn?RGB(0,240,255):RGB(50,54,70));HGDIOBJ sbp=SelectObject(dc,sbg);HGDIOBJ sbn=SelectObject(dc,GetStockObject(NULL_PEN));
  RoundRect(dc,swx,swy,swx+sww,swy+swh,18,18);SelectObject(dc,sbp);SelectObject(dc,sbn);DeleteObject(sbg);
  int bx=isOn?swx+sww-16:swx+2;
  HBRUSH ball=CreateSolidBrush(RGB(255,255,255));HGDIOBJ bbp=SelectObject(dc,ball);HGDIOBJ bbn=SelectObject(dc,GetStockObject(NULL_PEN));
  Ellipse(dc,bx,swy+2,bx+14,swy+16);SelectObject(dc,bbp);SelectObject(dc,bbn);DeleteObject(ball);
  y+=36;idx++;if(y>MENU_H-30)break;}
 SetTextColor(dc,RGB(80,86,110));TextOutA(dc,12,MENU_H-22,"CheatForge v10 | END exit",24);
}
static int featAt(int x,int y){if(y<94||x<8||x>MENU_W-8)return -1;int idx=0,yy=94;
 for(auto&f:g_feats){if(f.tab!=g_activeTab){idx++;continue;}if(y>=yy&&y<yy+34)return idx;yy+=36;idx++;}return -1;}
static void menuClick(int x,int y){
 if(x>=MENU_W-34&&y>=8&&y<32){g_menuOpen=false;ShowWindow(g_menu,SW_HIDE);return;}
...
static LRESULT CALLBACK MenuProc(HWND h,UINT m,WPARAM w,LPARAM l){
 ...
 if(m==WM_PAINT){PAINTSTRUCT ps;HDC wdc=BeginPaint(h,&ps);HDC dc=CreateCompatibleDC(wdc);
  ...
  drawMenuTo(dc);BitBlt(wdc,0,0,MENU_W,MENU_H,dc,0,0,SRCCOPY);SelectObject(dc,ob);DeleteObject(bm);DeleteDC(dc);
  EndPaint(h,&ps);return 0;}
 if(m==WM_ERASEBKGND)return 1;
 if(m==WM_LBUTTONDOWN){menuClick((short)LOWORD(l),(short)HIWORD(l));return 0;}
 if(m==WM_MOUSEMOVE){int ni=featAt((short)LOWORD(l),(short)HIWORD(l));if(ni!=g_hoverIdx){g_hoverIdx=ni;InvalidateRect(h,0,FALSE);}
  TRACKMOUSEEVENT tme={};tme.cbSize=sizeof(tme);tme.dwFlags=TME_LEAVE;tme.hwndTrack=h;TrackMouseEvent(&tme);return 0;}
...
int main(int argc,char**argv){
 ...
 if(!cfgEmpty&&CFG.find("\"features\": []")!=std::string::npos){return 1;}
 ...
 WNDCLASSA wc={};wc.lpfnWndProc=OvProc;wc.hInstance=GetModuleHandleA(0);wc.lpszClassName="EdgeUiWindow";RegisterClassA(&wc);
 ...
 WNDCLASSA mc={};mc.lpfnWndProc=MenuProc;mc.hInstance=GetModuleHandleA(0);mc.lpszClassName="CheatForgeMenu";mc.hbrBackground=0;RegisterClassA(&mc);
 g_menu=CreateWindowExA(WS_EX_TOPMOST|WS_EX_TOOLWINDOW,"CheatForgeMenu","",WS_POPUP,MENU_X,MENU_Y,MENU_W,MENU_H,0,0,mc.hInstance,0);
 ShowWindow(g_menu,SW_SHOW);UpdateWindow(g_menu);
 if(!keepConsole)FreeConsole();
#ifdef GAME_CS2
 Mem mem;uintptr_t base=0;bool attached=false;DWORD lastTry=0;
 if(mem.attach("cs2.exe")){attached=true;base=mem.modBase("client.dll");}
#else
 bool attached=true;
#endif
 if(!findPidWin(G.proc)&&!attached){MessageBoxA(NULL,"Game is not running.\nThe cheat attaches automatically on launch.\n\nINSERT = menu | END = exit","CheatForge",MB_OK|MB_ICONWARNING);}
 ...
 static DWORD mt[16];static bool mh[16];static int mflip[16];bool prevIns=false;
 while(!(GetAsyncKeyState(VK_END)&0x8000)){
  ...
  if(ins&&!prevIns){g_menuOpen=!g_menuOpen;ShowWindow(g_menu,g_menuOpen?SW_SHOW:SW_HIDE);}prevIns=ins;
  std::vector<DI> items;std::vector<Tgt> tg;bool crossHit=false;
...
#endif
#ifdef GAME_CS2
  if(!attached&&GetTickCount()-lastTry>1000){lastTry=GetTickCount();if(mem.attach("cs2.exe")){attached=true;base=mem.modBase("client.dll");}}
  if(attached){
   ...
   if(on("m_rcs")&&local){int sh=mem.rd<int>(local+OFF_SHOTS);float px=mem.rd<float>(local+OFF_PUNCH),py=mem.rd<float>(local+OFF_PUNCH+4);
    if(sh>prevShots){INPUT in={};in.type=INPUT_MOUSE;in.mi.dwFlags=MOUSEEVENTF_MOVE;float k=on("m_rcsh")?1.f:2.f;
     in.mi.dx=(LONG)-((px-ppx)*k);in.mi.dy=(LONG)-((py-ppy)*k);SendInput(1,&in,sizeof(in));
   ...
   if(on("m_bhop")&&local){int fl=mem.rd<int>(local+OFF_FLAGS);bool want=space&&(fl&1);if(want!=lj){mem.wr<int>(base+OFF_FORCEJUMP,want?6:0);lj=want;}}
   ...
   if(on("m_radar")&&local){float ca=cosf(yaw),sa=sinf(yaw);
    for(auto&t:tg){float dx=t.wx-lx,dy=t.wy-ly;DI d={3,g_sw-110+(int)((dx*ca-dy*sa)/40),110-(int)((dx*sa+dy*ca)/40),0,0,RGB(255,60,90),""};items.push_back(d);}}
  }
#endif
  for(auto&t:tg){int bx=(int)t.x-(int)t.w/2,by=(int)t.y,bw=(int)t.w,bh=(int)t.h;
...
#endif
#ifdef FEAT_D_CORNER
    if(on("d_corner")){int e=bh/4;DI L[8]={{1,bx,by,bx+e,by,col,0},{1,bx,by,bx,by+e,col,0},{1,bx+bw,by,bx+bw-e,by,col,0},{1,bx+bw,by,bx+bw,by+e,col,0},{1,bx,by+bh,bx+e,by+bh,col,0},{1,bx,by+bh,bx,by+bh-e,col,0},{1,bx+bw,by+bh,bx+bw-e,by+bh,col,0},{1,bx+bw,by+bh,bx+bw,by+bh-e,col,0}};for(auto&d:L)items.push_back(d);}
    else
#endif
  ...
  for(auto&t:tg){if(t.ore)continue;float ax=t.x,ay=on("m_head")||on("c_head")?t.y+8:t.y+t.h/2;
   float dx=ax-g_sw/2,dy=ay-g_sh/2,d=sqrtf(dx*dx+dy*dy);float lim=on("m_fov")||on("c_fov")?120:260;
